...
| **Fire & Thermal** | **VIIRS Thermal Hotspots & Fire Activity**<br/>`https://arcg.is/miur8` | **Live deployed** in `frontend/src/utils/arcGisLayers.js` + `MapView.jsx` as a current-map-extent FeatureServer query. The demo pipeline uses the same event model and clearly labeled simulated EO metrics; direct FeatureServer-to-engine ingestion is a production roadmap item. | Provides live wildfire situational awareness; payout requires verified fire-area metrics and configured settlement. |
...
|:---|:---|:---|:---|
| **ACRE Africa / Pula Advisors** | `https://startuplist.africa/` | Uses manual yield audits or picture-based insurance requiring 4-8 weeks processing. | **Parametric Automation Target**: GNSS/WGS84 farm evidence plus EO signals can reduce routine verification and target minute-level settlement. |
| **Apollo Agriculture** | `https://startuplist.africa/` | Bundles micro-credit with weather index but lacks auditable evidence transparency. | **Evidence + SMS**: SHA-256 claim evidence, optional ERC-20 settlement, and low-bandwidth status alerts for non-smartphones. |
